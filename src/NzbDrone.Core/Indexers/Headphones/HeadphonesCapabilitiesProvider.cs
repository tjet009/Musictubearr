...
namespace NzbDrone.Core.Indexers.Headphones
{
    ...
    public class HeadphonesCapabilitiesProvider : IHeadphonesCapabilitiesProvider
    {
        ...
        private HeadphonesCapabilities FetchCapabilities(HeadphonesSettings indexerSettings)
        {
            ...
            request.Credentials = new BasicNetworkCredential(indexerSettings.Username, indexerSettings.Password);
            ...
            {
                _logger.Error(ex, "Failed to determine headphones api capabilities for {0}, using the defaults instead till MusicTubearr restarts", indexerSettings.BaseUrl);
            }
