...
namespace NzbDrone.Core.Configuration
{
    ...
    public class ConfigFileProvider : IConfigFileProvider
    {
        ...
        public int Port => _serverOptions.Port ?? GetValueInt("Port", 8585);

        public int SslPort => _serverOptions.SslPort ?? GetValueInt("SslPort", 8586);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public string PostgresMainDb => _postgresOptions?.MainDb ?? GetValue("PostgresMainDb", "musictubearr-main", persist: false);
        public string PostgresLogDb => _postgresOptions?.LogDb ?? GetValue("PostgresLogDb", "musictubearr-log", persist: false);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public XDocument LoadConfigFile()
        {
            try
            {
                lock (Mutex)
                {
                    if (_diskProvider.FileExists(_configFile))
                    {
                        ...
                        {
                            throw new InvalidConfigFileException($"{_configFile} is empty. Please delete the config file and MusicTubearr will recreate it.");
                        }

                        if (contents.All(char.IsControl))
                        {
                            throw new InvalidConfigFileException($"{_configFile} is corrupt. Please delete the config file and MusicTubearr will recreate it.");
                        }
                        ...
                        if (config.Count != 1)
                        {
                            throw new InvalidConfigFileException($"{_configFile} is invalid. Please delete the config file and MusicTubearr will recreate it.");
                        }
            ...
            {
                throw new InvalidConfigFileException($"{_configFile} is corrupt is invalid. Please delete the config file and MusicTubearr will recreate it.", ex);
            }
            catch (UnauthorizedAccessException ex)
            {
                throw new AccessDeniedConfigFileException($"MusicTubearr does not have access to config file: {_configFile}. Please fix permissions", ex);
            }
        }
